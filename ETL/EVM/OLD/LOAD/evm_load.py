...
#INSERT GAS_METADATA
    ...
    df_gas_metadata.to_sql(con=conn, name="GAS_METADATA", if_exists='append', index=False)

    #INSERT TOKENS
    tokens = f"{wd}/EVM/TRANSFORM/files/tokens.csv"
    df_tokens = pd.read_csv(tokens)
    truncate_table("TOKENS", conn)
    df_tokens.to_sql(con=conn, name="TOKENS", if_exists='append', index=False)

    #INSERT LOGS
    logs = f"{wd}/EVM/TRANSFORM/files/logs.csv"
    df_logs = pd.read_csv(logs)
    truncate_table("LOGS", conn)
    df_logs.to_sql(con=conn, name="LOGS", if_exists='append', index=False)
    ...
    df_params_address.to_sql(con=conn, name="PARAMS_ADDRESS", if_exists='append', index=False)
    ...
    df_params_value.to_sql(con=conn, name="PARAMS_VALUES", if_exists='append', index=False)
    ...
    df_transactions.to_sql(con=conn, name="TRANSACTIONS", if_exists='append', index=False)
