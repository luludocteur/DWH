...
--
-- Dumping events for database 'DISTRIBUTION'
--

--
-- Dumping routines for database 'DISTRIBUTION'
--
...
begin
    ...
    delete from distribution.source;
    
    insert into distribution.source
...
begin
    ...
    delete from distribution.token;
    
    insert into distribution.token
	select id_token, ticker, logo from mapping_evm.token
union
select 
id_token as ID_TOKEN,
token as TOKEN,
null as LOGO
from mapping_binance.token
where id_token not in (select id_token from mapping_evm.token);
